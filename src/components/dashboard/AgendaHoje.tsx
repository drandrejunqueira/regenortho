        ...
        return (
          <li key={apt.id} className={`flex items-start gap-3 ${isCompleted ? 'opacity-50' : ''}`}>
            <div className="flex flex-col items-center">
              <span className="font-technical text-xs text-[#e6c364] font-bold w-10 text-center shrink-0">
                {timeStr}
              </span>
            </div>
            <div className="flex-1 min-w-0 border-l-2 border-[#61d8dd] pl-3">
              <p className="text-sm font-bold text-[#dfe2eb] truncate">{patientName}</p>
              <p className="text-xs text-[#bec9c9]">{APPOINTMENT_TYPE_LABELS[apt.type]}</p>
            </div>
          </li>
        )
      })}
